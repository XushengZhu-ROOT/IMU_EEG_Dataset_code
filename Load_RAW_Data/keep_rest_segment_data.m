function [EEG_kept, acc_data_kept, gyro_data_kept] = keep_rest_segment_data(EEG, acc_data, gyro_data, sub_idx, task, skip_channels)
...
% Set default values for optional parameters
    ...
    start_marker = 'S pressed';
    end_marker = 'S released';
    start_print = true;
    end_print = true;
    % Find all start and end event marker indices
    start_idx = find(strcmp({EEG.event.type}, start_marker));
    end_idx = find(strcmp({EEG.event.type}, end_marker));
    ...
    if isempty(end_idx)
        end_markers = {'1 pressed', '2 pressed', '3 pressed'};
        for m = 1:length(end_markers)
            end_marker = end_markers{m};
            idx = find(strcmp({EEG.event.type}, end_marker));
        ...
        fprintf('\n[Warning] No matching event markers found.\n');
    end
    ...
    if isfield(EEG, 'accelerometer') && isfield(EEG.accelerometer, 'data')
        ...
        fprintf('REST CGX acc length: %d samples\n', new_acc_samples);
    end
    ...
    after_fig = plotEEGData(sub_idx, [task ' - After Processing (REST Segment)'], EEG_kept, skip_channels, ...
    ...
    saveEEG_dir  = fullfile('EEG_Data', sub_idx, 'Segments', [sub_idx,'_', task, '_', 'Rest.mat']);
    ...
    saveACC_dir  = fullfile('IMU_Data', sub_idx, 'Segments', [sub_idx,'_', task, '_', 'ACC_Rest.mat']);
    ...
    saveGYRO_dir  = fullfile('IMU_Data', sub_idx, 'Segments', [sub_idx,'_', task, '_', 'GYRO_Rest.mat']);
    save(saveGYRO_dir, 'gyro_data');

    fprintf('save Rest.mat files to:\n%s\n%s\n%s', saveEEG_dir, saveACC_dir, saveGYRO_dir);
end